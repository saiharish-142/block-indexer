...
import (
	...
	"github.com/example/block-indexer/services/dag-indexer/internal/api"
	"github.com/example/block-indexer/services/dag-indexer/internal/pipeline"
...
func main() {
	...
	dagClient := rpc.NewDAGClient(cfg.RPC.DAGEndpoint)

	repos := repository.NewRepositories(pool)
	p := pipeline.NewManager(cfg, log, repos, dagClient, natsClient)
	p.Start(ctx)
	...
	log.Info("dag-indexer starting", zap.Int("port", cfg.HTTPPort))
...
func newRouter(log *zap.Logger, p *pipeline.Manager) *chi.Mux {
	...
	r.Route("/api/v1/dag", func(r chi.Router) {
